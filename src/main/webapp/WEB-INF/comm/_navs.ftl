<li id="_apidocs"><a href="/nav/">网址导航</a></li>
<li id="_apidocs"><a href="/apidocs/">API文档</a></li>

<li class="dropdown " id='_contrast'>
    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
        对照表 <b class="caret"></b>
    </a>
    <ul class="dropdown-menu">
        <li><a href="/contrast/hct">HTTP Content-type</a></li>
        <li><a href="/contrast/hec">HTML转义字符</a></li>
        <li><a href="/contrast/rgb">RGB颜色参考</a></li>
        <li><a href="/contrast/ascii">ASCII对照表</a></li>
        <li><a href="/contrast/hsc">Http状态码详解</a></li>
        <li><a href="/contrast/tup">TCP/UDP端口</a></li>
    </ul>
</li>

<li class="dropdown " id='_format'>
    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
        代码处理<b class="caret"></b>
    </a>
    <ul class="dropdown-menu">
        <li><a href="/format/css">CSS格式化</a></li>
        <li><a href="/format/js">JS格式化</a></li>
        <li><a href="/format/xml">XML格式化</a></li>
    </ul>
</li>

<li class="dropdown " id='_encrypt'>
    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
        加密转码 <b class="caret"></b>
    </a>
    <ul class="dropdown-menu">
        <li><a href="/encrypt/ecp">通用加密工具</a></li>
        <li><a href="/encrypt/morse">摩斯密码翻译工具</a></li>
        <li><a href="/encrypt/barcode">条码生成器</a></li>
        <li><a href="/encrypt/ideaKeygen">IDEA注册码生成器</a></li>

    </ul>
</li>


<li class="dropdown " id='_life'>
    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
        便民 <b class="caret"></b>
    </a>
    <ul class="dropdown-menu">

        <li><a href="/life/kuaidi">快递查询</a></li>
        <li><a href="/life/bank">银行卡信息查询</a></li>
        <!-- <li><a href="/life/idcard">身份证信息校验</a></li> -->
        <li><a href="/life/weather">天气查询</a></li>
        <li><a href="/life/wxeditor">微信编辑器</a></li>
        <li><a href="/life/qrcode">二维码生成器</a></li>
        <li><a href="/life/meitups">美图秀秀</a></li>
    </ul>
</li>

<!-- 
<li class="dropdown ">
   <a href="#" class="dropdown-toggle" data-toggle="dropdown">
      	站长工具<b class="caret"></b>
   </a>
   <ul class="dropdown-menu" >
   		<li><a href="/site/url">短链</a></li>
   		
   </ul>
</li>
 -->